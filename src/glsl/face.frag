...
void main(void) {
  ...
  // vec4 dest = vec4(colorA, 1.0);
  // if((vUv.y > pA.y && vUv.x > pA.x) || (vUv.y > pB.y && vUv.x > pB.x)) {
  //   dest.rgb = colorB;
  // }
  // gl_FragColor = dest;
  ...
  vec3 dest = useColor;
  gl_FragColor = vec4(dest * light * (directionalLight + ambientLightColor), 1.0);

  if (abs(totalArea - area) < 0.001) {
